...
const SideBarUesr = ({ chat }) => {
  const { setCurrentChat } = useContext(ChatContext);
  const { isEditchatActive, choosedChatRoom, setChoosedChatRoom } =
  ...
  if (!getChatUserInfo.isPending) {
    ...
    return (
      ...
      >
        ...
        {isEditchatActive && (
          <span
            className={`absolute left-5 flex w-[2rem] h-[2rem] ${choosedChatRoom?.chat_id == chat.id ? "bg-gray-300 dark:bg-gray-500" : ""}  rounded-full border-2 dark:border-gray-500 border-gray-300`}
          ></span>
        )}
      </div>
    );

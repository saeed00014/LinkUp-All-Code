...
const Message = ({ message }: { message: ChatMessageType }) => {
  ...
  const getSentPostInfo = useQuery({
    ...
    queryFn: async () => {
      if (message.post_id) {
        const post = await baseURL.get(`/post/${message.post_id}`);
        return post;
      }
      return null;
    },
  ...
  const handleAttachedMessage = () => {
    const attachedMessage = {
      id: message.attachedMessage_id,
      user_id: 0,
      text: message.attachedMessage,
      post_id: message.post_id,
      image: message.image,
      attachedMessage_id: 0,
      attachedMessage: "",
    };
    const isAttachedMessageChoosed = chooseMessage.id == attachedMessage.id;
    const isAttachedMessage = messages.find(
      (message) => message.id == attachedMessage.id
    );
    !isAttachedMessage && setChooseMessage({} as ChatMessageType);
    isAttachedMessage &&
      !isAttachedMessageChoosed &&
      setChooseMessage(attachedMessage);
    isAttachedMessage &&
      isAttachedMessageChoosed &&
      setChooseMessage({} as ChatMessageType);
  };
  ...
  if (getSentPostInfo.error) {
    <></>;
  }

  const user = message.user_id === loginUser.id ? loginUser : targetUser;
  const post: PostType =
    getSentPostInfo?.data?.data?.response[0] || ({} as PostType);
          ...
          >
            ...
            {message.post_id && !message.attachedMessage_id ? (
              <div id={`message${message.id}`} onClick={handleClick}>
                <Post post={post} miniEdition={true} />
              </div>
            ) : null}
          </div>
        </div>

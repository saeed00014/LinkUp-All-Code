...
export async function GET() {
  ...
  const result = await query({
    query:
      "SELECT id,username,firstname,image FROM user WHERE id != ? ORDER BY RAND() LIMIT 2",
    values: [loginUser.id],
  });
...
export async function POST(req) {
  ...
  const result = await query({
    query:
      "INSERT INTO user (email, username, firstname, lastname, gender, birth, password) VALUES (?, ?, ?, ?, ?, ?, ?)",
    values: values,
  });
  if (result.insertId) {
    return NextResponse.json(
      { id: result.insertId, username: username, password: hashedPassword },
      { status: 200 }
    );
  }
  ...
  return NextResponse.json(
    { message: "your request failed pls try later" },
    { status: 500 }
  );
}

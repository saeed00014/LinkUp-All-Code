...
export async function GET(req, route) {
  ...
  if(result[0] && !result.errno) {
    return NextResponse.json({ response: "user followed", isFollowed: true, id: result[0].id }, { status: 200 })
  }
  if(!result[0] && !result.errno) {
    return NextResponse.json({ response: "user notFollowed", isFollowed: false, id: "" }, { status: 200 })
  }
  if(result) {

...
const PostShareUser = ({user}) => {
  ...
  const handleChooseUser = () => {
    ...
    isUserChoosed && setChooseUsers(oldUsers => oldUsers.filter((oldUser) => oldUser.id != user.id))
  ...
  return (
    ...
    >
      ...
      <span className={`absolute left-5 flex w-[2rem] h-[2rem] ${isUserChoosed ? "bg-gray-300 dark:bg-gray-500" : ""} rounded-full border-2 dark:border-gray-500 border-gray-300`}></span>
    </div>
  )
}

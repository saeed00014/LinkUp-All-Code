...
const Post = () => {
  return (
    <article className='flex flex-col min-w-[550px] rounded-[1rem] bg-white dark:bg-gray-800 dark:text-white gap-1 py-3'>
      <header className='flex justify-between pl-6 pr-4 text-[.9rem]'>
        <Link href={"/profile"} className='flex gap-4'>
          ...
          <div className='flex flex-col'>
            <span>یاسمین</span>
            <span>yasamin043</span>
          </div>
        </Link>
        <EditInfoButton />
      </header>
      <div className='flex flex-col gap-3 text-[.9rem]'>
        <p className='px-4'>{register.passRepeatedErorr}</p>
      ...
      <footer className='flex flex-col px-4 gap-2 mt-1'>
        ...
        <div className='flex justify-between items-center py-1 border-t-2 border-b-2 dark:border-gray-600 gap-1'>
          <PostLike />
          <PostComment />
          <PostShare />
        </div>
        <div className='flex w-full py-2'>
          <p>{register.passRepeatErorr}</p>
        </div>
      </footer>
    </article>
  )

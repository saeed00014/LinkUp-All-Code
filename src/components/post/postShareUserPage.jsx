        ...
        <div className="flex flex-col w-full max-w-[550px] bg-white dark:bg-gray-800 rounded-[1rem]">
          ...
          <div className="flex flex-col h-screen min-h-full max-h-[25rem] bg-gray-900 gap-1">
            ...
            <div className="flex flex-col overflow-y-scroll">
              ...
              {(!isPending && !loginUserFreind.isPending) && !searchValue && loginUserFreind.data.map((user) => {
                return (
                  <div key={user.id}>
                    <PostShareUserResultUserInfo 
                      user={user}
                    />
                  </div>
                )
              })}
              {(!isPending && !loginUserFreind.isPending) && searchResult && searchResult.map((user) => {
                return (
                  <div key={user.id}>
